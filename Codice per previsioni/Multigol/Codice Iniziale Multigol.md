...
intervalli_gol = {
    "multigol_1_3": range(1, 4),  # Gol totali tra 1 e 3
    "multigol_2_4": range(2, 5),  # Gol totali tra 2 e 4
    "multigol_3_5": range(3, 6),  # Gol totali tra 3 e 5
    "multigol_4_6": range(4, 7),  # Gol totali tra 4 e 6
...
# Sezione per calcolare il valore atteso (EV) per un intervallo specifico con una quota
def calcola_valore_atteso(probabilita):
	return (0.1 + (1 - probabilita)) / probabilita
    #return (probabilita * quota) - (1 - probabilita)

# Esempio di calcolo del valore atteso per l'intervallo "multigol_2_4" con una quota di 1.6
#quota = 1.6
for item in intervalli_gol.keys():
	prob_multigol_2_4 = probabilita_intervalli[item]
	ev_multigol_2_4 = calcola_valore_atteso(prob_multigol_2_4)
	print(f"\nQuota ottimale per {item}: {ev_multigol_2_4:.4f}")
